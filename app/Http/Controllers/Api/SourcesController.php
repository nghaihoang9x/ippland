...
class SourcesController extends ApiController
{
    public function store(Request $request)
    {
        ...
        $ServiceModel = new Source();
        $result = $ServiceModel->saveSource($data);
        if ($result) {
            $res['_id'] = $result;
    ...
    public function list(Request $request)
    {
        $get = $request->all();
        $ServiceModel = new Source();
        $result = $ServiceModel->listSource($get);
    ...
    public function update(Request $request, $id)
    {
        ...
        $ServiceModel = new Source();
        $result = $ServiceModel->updateSource($data, $id);
        if ($result) {
            $res['response_time'] = microtime(true) - LARAVEL_START;
            $res['request'] = $request->all();
            return $this->responseSuccess($res);
    ...
    public function updateMultiple(Request $request)
    {
        ...
        foreach ($post as $key => $value) {
            ...
            $result = $ServiceModel->updateSource($data, $key);
        }
    ...
    public function delete(Request $request) {
        ...
        if ($ids) {
            ...
            foreach ($ids as $_id) {
                $data['deleted'] = 1;
                $ServiceModel->updateSource($data, $_id);
            }
