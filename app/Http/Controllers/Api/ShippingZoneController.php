...
class ShippingZoneController extends ApiController
{
    public function store(Request $request)
    {
        ...
        $ServiceModel = new Zone();
        $result = $ServiceModel->saveZone($data);
        if ($result) {
            $res['_id'] = $result;
    ...
    public function list(Request $request)
    {
        $get = $request->all();
        $ServiceModel = new Zone();
        $result = $ServiceModel->listZone($get);
    ...
    public function update(Request $request, $id)
    {
        ...
        $ServiceModel = new Zone();
        $result = $ServiceModel->updateZone($data, $id);
        if ($result) {
            $res['response_time'] = microtime(true) - LARAVEL_START;
            $res['request'] = $request->all();
            return $this->responseSuccess($res);
    ...
    public function updateMultiple(Request $request)
    {
        ...
        foreach ($post as $key => $value) {
            ...
            $result = $ServiceModel->updateZone($data, $key);
        }
    ...
    public function delete(Request $request) {
        ...
        if ($ids) {
            ...
            foreach ($ids as $_id) {
                $data['deleted'] = 1;
                $ServiceModel->updateZone($data, $_id);
            }
