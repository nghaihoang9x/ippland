...
class InventoryController extends ApiController
{
    public function store(Request $request)
    {
        ...
        $ServiceModel = new Inventory();
        $result = $ServiceModel->saveInventory($data);
        if ($result) {
            $res['_id'] = $result;
    ...
    public function list(Request $request)
    {
        $get = $request->all();
        $ServiceModel = new Inventory();
        $result = $ServiceModel->listInventory($get);
    ...
    public function update(Request $request, $id)
    {
        ...
        $ServiceModel = new Inventory();
        $result = $ServiceModel->updateInventory($data, $id);
        if ($result) {
            $res['response_time'] = microtime(true) - LARAVEL_START;
            $res['request'] = $request->all();
            return $this->responseSuccess($res);
    ...
    public function updateMultiple(Request $request)
    {
        ...
        foreach ($post as $key => $value) {
            ...
            $result = $ServiceModel->updateInventory($data, $key);
        }
    ...
    public function delete(Request $request) {
        ...
        if ($ids) {
            ...
            foreach ($ids as $_id) {
                $data['deleted'] = 1;
                $ServiceModel->updateInventory($data, $_id);
            }
