...
class WarehouseController extends ApiController
{
    public function store(Request $request)
    {
        ...
        $data['_id'] = $objectID;
        $data['id'] = $objectID;
        $ServiceModel = new Warehouse();
        $result = $ServiceModel->saveWarehouse($data);
        if ($result) {
            $res['_id'] = $result;
    ...
    public function list(Request $request)
    {
        $get = $request->all();
        $ServiceModel = new Warehouse();
        $result = $ServiceModel->listWarehouse($get);
    ...
    public function update(Request $request, $id)
    {
        ...
        $ServiceModel = new Warehouse();
        $result = $ServiceModel->updateWarehouse($data, $id);
        if ($result) {
            $res['response_time'] = microtime(true) - LARAVEL_START;
            $res['request'] = $request->all();
            return $this->responseSuccess($res);
    ...
    public function updateMultiple(Request $request)
    {
        ...
        foreach ($post as $key => $value) {
            ...
            $result = $ServiceModel->updateWarehouse($data, $key);
        }
    ...
    public function delete(Request $request) {
        ...
        if ($ids) {
            ...
            foreach ($ids as $_id) {
                $data['deleted'] = 1;
                $ServiceModel->updateWarehouse($data, $_id);
            }
